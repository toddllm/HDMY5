...
export interface DataService {
    ...
    exportGame(): Promise<string>;
    importGame(gameData: string): Promise<GameScene[]>;
}
...
export class LocalStorageService implements DataService {
    ...
    async exportGame(): Promise<string> {
        ...
        // Create a game data object with metadata
        const gameData = {
            version: '1.0',
    ...
    async importGame(gameData: string): Promise<GameScene[]> {
        try {
            const data = JSON.parse(gameData);
            
            // Check if it's a valid game data format
            if (!data.scenes || !Array.isArray(data.scenes)) {
                throw new Error('Invalid game data format');
            }
            
            // Save the imported scenes
            localStorage.setItem(this.STORAGE_KEY, JSON.stringify(data.scenes));
