    ...
    for (let x = 0; x < voxelData1.width; x++) {
      for (let y = 0; y < voxelData1.height; y++) {
        for (let z = 0; z < voxelData1.depth; z++) {
          expect(voxelData1.getBlock(x, y, z)).toBe(
            voxelData2.getBlock(x, y, z)
          );
        }
      }
    }
      ...
      for (let z = 0; z < voxelData.depth; z += 4) {
        ...
        for (let y = voxelData.height - 1; y >= 0; y--) {
          ...
          if (!foundSurface && block !== BlockType.AIR) {
            // First non-air block could be grass or other surface blocks
            ...
            // Next few blocks could be dirt or other materials
            expect(block).not.toBe(BlockType.AIR);
            dirtLayerCount++;
          } else if (
            foundSurface &&
            ...
            // Lower blocks are typically stone or other materials
            expect(block).not.toBe(BlockType.AIR);
          }
        }
