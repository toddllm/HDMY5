...
export class TerrainGenerator {
  ...
  generateTerrain(
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let z = 0; z < depth; z++) {
        ...
        const temperatureNoise = this.noise2D(
          worldX * 0.008 + 500,
          worldZ * 0.008 + 500
        );
        ...
        heightMap[x][z] = Math.max(
          1,
          Math.min(Math.floor(terrainHeight), height - 20)
        );
        biomeMap[x][z] = biome;
      }
    }
    ...
    for (let lx = -leafRadius; lx <= leafRadius; lx++) {
      for (let ly = -leafRadius; ly <= leafRadius; ly++) {
        for (let lz = -leafRadius; lz <= leafRadius; lz++) {
          ...
          if (
            x + lx < 0 ||
            x + lx >= voxelData.width ||
            y + treeHeight - 1 + ly < 0 ||
            y + treeHeight - 1 + ly >= voxelData.height ||
            z + lz < 0 ||
            z + lz >= voxelData.depth
          ) {
            continue;
          }
  ...
  private generateHeight(x: number, z: number): number {
    ...
    for (let i = 0; i < 4; i++) {
      noise +=
        this.noise2D(x * noiseScale * frequency, z * noiseScale * frequency) *
        amplitude2;
      amplitude2 *= 0.5;
      frequency *= 2;
    }
