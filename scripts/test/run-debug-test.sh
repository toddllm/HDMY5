...
# Clean up any existing debug screenshots
rm -f debug-*.png
...
echo "Running debug test with Xvfb..."
xvfb-run --auto-servernum --server-args="-screen 0 1280x960x24" npx playwright test tests/visual-test-debug.spec.ts --project=chromium

echo "Copying screenshots to $SCREENSHOTS_DIR..."
for file in debug-*.png; do
    if [ -f "$file" ]; then
...
<head>
    ...
    <title>Debug Test Screenshots</title>
    <style>
        body {
...
for file in "$SCREENSHOTS_DIR"/*.png; do
    if [ -f "$file" ]; then
        ...
        title=$(echo "$filename" | sed 's/debug-//' | sed 's/\.png$//' | sed 's/-/ /g' | sed 's/\b\(.\)/\u\1/g')
        
        cat >> "$SCREENSHOTS_DIR/index.html" << EOL
        <div class="screenshot">
            <h3>$title</h3>
            <img src="$filename" alt="$title">
            <p>Debug screenshot from test.</p>
        </div>
EOL
    fi
done
...
cd "$SCREENSHOTS_DIR" && npx http-server -p 8081 &
HTTP_SERVER_PID=$!

echo "Screenshots available at http://localhost:8081/"
...
trap "kill $HTTP_SERVER_PID 2>/dev/null" EXIT
wait 