...
echo "Killing processes using ports 5173 and 8082..."
...
echo "Running button debug test with Xvfb..."
xvfb-run --auto-servernum --server-args="-screen 0 1280x960x24" npx playwright test tests/button-debug.spec.ts --project=chromium
...
<head>
    ...
    <title>Button Debug Screenshots</title>
    <style>
        body {
...
<body>
    <h1>Button Debug Screenshots</h1>
...
for file in "$SCREENSHOTS_DIR"/*.png; do
    if [ -f "$file" ]; then
        ...
        <div class="screenshot">
            ...
            <p>Screenshot from the button debug test.</p>
        </div>
EOL
    fi
done
...
cd "$SCREENSHOTS_DIR" && npx http-server -p 8082 &
HTTP_SERVER_PID=$!

echo "Screenshots available at http://localhost:8082/"
